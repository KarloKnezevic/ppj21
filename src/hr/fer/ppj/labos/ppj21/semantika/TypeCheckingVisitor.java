...
public class TypeCheckingVisitor implements Visitor {
	...
	private String cPath, tType;
	boolean typeCheckErrorEncountered;

	public TypeCheckingVisitor(Map<String, String> symTable) {
	...
	public String getType(String varName){
		...
		while(tPath.length()>0){
			if(symTable.containsKey(tPath+":"+varName))
				return symTable.get(tPath+":"+varName);
			int itemp = tPath.lastIndexOf(":");
			if(itemp==-1)
				tPath = tPath.substring(0, 0);
			else
				tPath = tPath.substring(0, itemp);
		}
	...
	public void visit(MethodDecl n) throws Exception {
		String tempPath = cPath;
		//na path se dodaje : i ime metode
		cPath += ":"+n.f2.f0.tokenImage;
	...
	public void visit(Type n) throws Exception {
		...
		if(n.f0.which==3){
			String tName = symTable.get(((ID)n.f0.choice).f0.tokenImage);
			//ako ne postoji takva klasa, javi grešku
			if(tName==null || !tName.equals("class"))
		...
		try {
			...
			switch(n.f0.which){
				...
				case 3:
					...
					if(!tType.equals("int") && !tType.equals("boolean") && !tType.equals("string"))
	...
	public void visit(Factor n) throws Exception {
		...
		if(n.f0.present()){
			...
			if(tType.equals("int") || tType.equals("int[]") || tType.equals("string[]"))
	...
	public void visit(MethodCall n) throws Exception {
		if(n.f0.which==0){
			...
			if(tType.equals("int") || tType.equals("int[]") || tType.equals("boolean") || tType.equals("string") || tType.equals("string[]"))
			...
			String tMethod = symTable.get(tType + ":" + ((ID)v.elementAt(1)).f0.tokenImage);
			if(tMethod==null)
	...
	public void visit(ID n) throws Exception {
		String tName = getType(n.f0.tokenImage);
